...
//        }
        ...
        $getters = [];
        $magicMethods = [];
        $skipMagicMethods = array('id', 'create_time', 'update_time', 'create_by', 'update_by', 'status');
        foreach ($fields as $field) {
            ...
            if (!in_array(strtolower($field->getName()), $skipMagicMethods)) {
                $setterName = Utils::camelize($field->getName());
                $magicMethods[] = $this->snippet->getMagicMethod($setterName, $field->getName());
            }
        }
...
//        }
        ...
        $classDoc = '';
        if ($genDocMethods) {
            $magicMethodContent = join("", $magicMethods);
            $classDoc = $this->snippet->getClassDoc($className, $namespace, $magicMethodContent);
        }
        ...
        if ($this->isConsole()) {
            ...
            printf($msgSuccess . PHP_EOL, $this->options->get('name'));
        }
    }
}
