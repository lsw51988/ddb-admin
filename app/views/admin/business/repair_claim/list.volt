            ...
            <div class="layui-row layui-form-item ">
                <div class="layui-col-md12">
                    ...
                    <div class="layui-input-block">
                        <input type="radio" name="status" value="" title="请选择"
                               {% if search['status']=='' %}checked{% endif %}>
                        <input type="radio" name="status" value="1" title="刚创建"
    ...
    <table class="layui-table">
        ...
        {% for item in data %}
            <tr>
                ...
                <td>{{ item['address'] }}</td>
                <td>{{ item['mobile'] }}</td>
                <td>{% if item["status"]==1 %}被创建{% elseif item["status"]==2 %}后台审核通过 {% else %}后台审核拒绝{% endif %}</td>
                <td>{{ item['reason'] }}</td>
        ...
        layui.use(['laypage', 'jquery', 'form', 'layer'], function () {
            ...
            laypage.render({
                ...
                , jump: function (obj, first) {
                    if (!first) {
                        window.location.href = "/admin/business/repairClaim/list?page=" + obj.curr
                    }
                }
            });
            ...
            $("#reset").click(function () {
                window.location.href = "/admin/business/repairClaim/list";
            });
